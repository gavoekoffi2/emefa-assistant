...
class SkillRegistry(ScopedStore):
    ...
    def for_scope(self, scope: Scope) -> "SkillRegistry":
        return type(self)(self.database_path, self.catalogue_path, self.available_tools, scope)
    ...
    def enabled_names(self) -> set[str]:
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT skill_name FROM enabled_skills WHERE tenant_id = ? AND user_id = ?",
    ...
    def enable(self, name: str) -> bool:
        ...
        with self._connect() as connection:
            connection.execute(
                "INSERT OR IGNORE INTO enabled_skills "
                "(skill_name, tenant_id, user_id) VALUES (?, ?, ?)",
                (name, self.scope.tenant_id, self.scope.user_id),
            )
        return True
    ...
    def disable(self, name: str) -> bool:
        with self._connect() as connection:
            removed = connection.execute(
                "DELETE FROM enabled_skills WHERE skill_name = ? AND tenant_id = ? AND user_id = ?",
                (name, self.scope.tenant_id, self.scope.user_id),
            ).rowcount
        return bool(removed)
