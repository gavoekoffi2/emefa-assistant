...
class MemoryRepository(ScopedStore):
    """Factual kernel facade with the Premium per-user explicit-memory view.

    Direct construction keeps the historical factual-kernel contract used by
    ingestion and consolidation. ``for_scope`` (the request path) binds the
    explicit memory tools to the authenticated owner, so Premium tenant
    isolation is preserved while the kernel migration remains compatible.
    """

    ownership = Ownership.USER

    def __init__(self, database_path: Path, scope: Scope | None = None) -> None:
        # The factual kernel is canonical for every scope. The former split
        # isolated only the legacy facade while events and facts stayed global.
        self._scoped_explicit = False
        super().__init__(database_path, scope)
        self.database_path = database_path
        self.kernel = MemoryKernel(database_path, self.scope)
    ...
    def search(self, query: str, limit: int = 8) -> list[Memory]:
        if self._scoped_explicit:
            ...
            return [item for item in self.list_all(limit=100) if needle in item.content.casefold()][
                :limit
            ]
    ...
    def context_block(self, max_items: int = 12, max_chars: int = 200, query: str = "") -> str:
        if self._scoped_explicit:
            entries = self.search(query, max_items) if query else self.list_all(max_items)
            if not entries:
                return ""
            lines = ["Mémoire durable (l'utilisateur peut la consulter et l'effacer) :"]
            lines.extend(f"- [{item.category}] {item.content[:max_chars]}" for item in entries)
            return "\n".join(lines)
