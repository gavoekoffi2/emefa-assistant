...
class InitiativeRepository(ScopedStore):
    ...
    def raise_initiative(self, initiative: Initiative) -> Initiative | None:
        ...
        with self._connect() as connection:
            try:
                connection.execute(
                    f"INSERT INTO proactive_initiatives (tenant_id, user_id, {_COLUMNS}) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (
    ...
    def get(self, initiative_id: str) -> Initiative | None:
        with self._connect() as connection:
            row = connection.execute(
                ...
                (initiative_id, self.scope.tenant_id, self.scope.user_id),
            ).fetchone()
    ...
    def open_initiatives(self, limit: int = 50) -> list[Initiative]:
        ...
        with self._connect() as connection:
            rows = connection.execute(
                f"SELECT {_COLUMNS} FROM proactive_initiatives WHERE tenant_id = ? AND user_id = ? "
                f"AND status IN ({placeholders}) "
                "ORDER BY created_at DESC LIMIT ?",
                (self.scope.tenant_id, self.scope.user_id, *OPEN_STATUSES, limit),
            ).fetchall()
    ...
    def history(self, limit: int = 50) -> list[Initiative]:
        with self._connect() as connection:
            rows = connection.execute(
                f"SELECT {_COLUMNS} FROM proactive_initiatives "
                "WHERE tenant_id = ? AND user_id = ? ORDER BY created_at DESC LIMIT ?",
                (self.scope.tenant_id, self.scope.user_id, limit),
            ).fetchall()
        return [_from_row(row) for row in rows]
    ...
    def set_status(self, initiative_id: str, status: InitiativeStatus) -> Initiative | None:
        ...
        with self._connect() as connection:
            updated = connection.execute(
                ...
                "AND tenant_id = ? AND user_id = ?",
                (status.value, resolved, initiative_id, self.scope.tenant_id, self.scope.user_id),
            ).rowcount
    ...
    def expire_overdue(self, now: str | None = None) -> int:
        ...
        with self._connect() as connection:
            expired = connection.execute(
                ...
                f"WHERE tenant_id = ? AND user_id = ? AND status IN ({placeholders}) AND deadline IS NOT NULL "
                "AND deadline < ?",
                (InitiativeStatus.EXPIRED.value, reference, self.scope.tenant_id,
                 self.scope.user_id, *OPEN_STATUSES, reference),
            ).rowcount
        return int(expired)
    ...
    def counts(self) -> dict[str, int]:
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT status, COUNT(*) AS total FROM proactive_initiatives "
