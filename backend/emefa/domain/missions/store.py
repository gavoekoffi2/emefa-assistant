...
class MissionRepository(ScopedStore):
    ...
    def create(
        ...
        with self._connect() as connection:
            connection.execute(
                f"INSERT INTO missions (tenant_id, user_id, {_MISSION_COLUMNS}) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
    ...
    def get(self, mission_id: str) -> Mission | None:
        with self._connect() as connection:
            row = connection.execute(
                ...
                (mission_id, self.scope.tenant_id, self.scope.user_id),
            ).fetchone()
            if row is None:
                return None
    ...
    def resumable(self, limit: int = 20) -> list[Mission]:
        ...
        with self._connect() as connection:
            rows = connection.execute(
                f"SELECT {_MISSION_COLUMNS} FROM missions WHERE tenant_id = ? AND user_id = ? "
                "AND status IN (?, ?) "
    ...
    def set_mission_status(
        ...
        with self._connect() as connection:
            updated = connection.execute(
                ...
                "WHERE mission_id = ? AND tenant_id = ? AND user_id = ?",
                (status.value, error[:1_000], _now(), mission_id,
                 self.scope.tenant_id, self.scope.user_id),
            ).rowcount
        return self.get(mission_id) if updated else None
    ...
    def update_step(
        ...
        with self._connect() as connection:
            connection.execute(
                ...
                "AND EXISTS (SELECT 1 FROM missions m WHERE m.mission_id = mission_steps.mission_id "
