...
export type ProactiveInitiative = {
  initiative_id: string
  type: string
  title: string
  reason: string
...
type ProactiveListing = { initiatives: ProactiveInitiative[]; counts: Record<string, number> }
type CuratorReport = { date: string; text: string; tokens_today: number; pricing_configured: boolean }
...
/** Proactive suggestions raised by EMEFA, kept distinct from user-managed
 * command-center initiatives because their lifecycle and governance differ. */
export function ProactiveInitiativesPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  const [listing, setListing] = useState<ProactiveListing | null>(null)
  const [curator, setCurator] = useState<CuratorReport | null>(null)
  const [error, setError] = useState('')
  const [busy, setBusy] = useState('')

  const reload = useCallback(() => {
    api<ProactiveListing>('/v1/initiatives')
      .then((data) => { setListing(data); setError('') })
      .catch((cause) => setError(cause instanceof Error ? cause.message : 'Chargement impossible.'))
    api<CuratorReport>('/v1/initiatives/curator').then(setCurator).catch(() => setCurator(null))
  }, [])

  useEffect(() => { if (open) { setListing(null); reload() } }, [open, reload])
  if (!open) return null

  const act = async (initiative: ProactiveInitiative, action: 'approve' | 'dismiss') => {
  ...
  return (
    <div className="profile-overlay" role="dialog" aria-modal="true" aria-labelledby="proactive-title">
      <section className="profile-panel">
        <header className="profile-head">
          <div><h2 id="proactive-title">Initiatives proactives</h2><p>Ce qu’EMEFA a remarqué sans qu’on le lui demande. Elle propose ; vous décidez.</p></div>
          <button className="profile-close" onClick={onClose} aria-label="Fermer les initiatives proactives">✕</button>
        </header>
        {error && <div className="form-error" role="alert">{error}</div>}
        {listing === null && !error && <p className="profile-status">Chargement…</p>}
        {listing !== null && listing.initiatives.length === 0 && <p className="profile-status">Rien à signaler. EMEFA analyse vos tâches, votre pipeline et sa mémoire à intervalle régulier.</p>}
        {listing?.initiatives.map((initiative) => (
          <div key={initiative.initiative_id} className="task-row initiative-row">
            <div>
              ...
              <strong>{initiative.title}</strong><small>{initiative.reason}</small>
              <small className="initiative-next">Prochaine étape : {initiative.next_action}</small>
              {initiative.requires_validation && <small className="initiative-gate">Votre accord est requis avant toute exécution.</small>}
            </div>
        ...
        {curator && <div className="task-group"><span className="profile-section">Entretien</span><pre className="curator-report">{curator.text}</pre></div>}
        <footer className="profile-actions">
          <button type="button" className="profile-later" onClick={() => void refresh()} disabled={busy === 'refresh'}>{busy === 'refresh' ? 'Analyse…' : 'Analyser maintenant'}</button>
