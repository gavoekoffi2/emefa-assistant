...
class EntityRepository:
    ...
    def upsert(
        ...
        *,
        ...
        with self._connect() as connection:
            connection.execute(
                f"INSERT INTO entities ({_ENTITY_COLUMNS}) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    entity.entity_id,
                    entity.kind.value,
    ...
    def get(self, entity_id: str) -> Entity | None:
        with self._connect() as connection:
            row = connection.execute(
                f"SELECT {_ENTITY_COLUMNS} FROM entities WHERE entity_id = ?", (entity_id,)
            ).fetchone()
        return _entity_from(row) if row is not None else None

    def find(self, kind: EntityKind | str | None, name: str) -> Entity | None:
        slug = slugify(name)
        query = f"SELECT {_ENTITY_COLUMNS} FROM entities WHERE slug = ?"
        parameters: list[Any] = [slug]
        if kind is not None:
            query += " AND kind = ?"
    ...
    def resolve(self, name: str, kind: EntityKind | str | None = None) -> Entity | None:
        ...
        if len(slug) < 3:
            return None
        query = f"SELECT {_ENTITY_COLUMNS} FROM entities WHERE slug LIKE ?"
        parameters: list[Any] = [f"%{slug}%"]
        if kind is not None:
            query += " AND kind = ?"
            parameters.append(normalise_kind(kind).value)
        with self._connect() as connection:
    ...
    def record_milestone(
        ...
        with self._connect() as connection:
            connection.execute(
                ...
                "(entry_id, entity_id, milestone, headline, occurred_at, event_id, created_at) "
                "VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
                    entry.entry_id,
                    entry.entity_id,
    ...
    def counts(self) -> dict[str, int]:
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT kind, COUNT(*) AS total FROM entities GROUP BY kind"
            ).fetchall()
