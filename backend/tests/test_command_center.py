...
def test_command_center_repositories_persist_and_detect_due_routines(tmp_path):
    ...
    # start_run persists SQLite CURRENT_TIMESTAMP. Keep both checks on the
    # actual current calendar day instead of comparing it to a fixed past date.
    routine_day = datetime.now(timezone.utc).replace(
        hour=9, minute=0, second=0, microsecond=0
    )
    assert routines.due(routine_day) == [daily]
    run = routines.start_run(daily.routine_id)
    routines.finish_run(run.run_id, "completed", "Revue prête")
    assert routines.due(routine_day.replace(minute=30)) == []
