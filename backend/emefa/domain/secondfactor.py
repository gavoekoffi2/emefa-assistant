...
class SecondFactorRepository:
    ...
    def register(
        ...
        with self._connect() as connection:
            connection.execute(
                ...
                "(credential_id, account_id, public_key, label, sign_count, created_at) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (
                    credential_id,
                    account_id,
    ...
    def get(self, credential_id: str) -> Credential | None:
        with self._connect() as connection:
            row = connection.execute(
                ...
                "created_at, last_used_at FROM webauthn_credentials WHERE credential_id = ?",
                (credential_id,),
            ).fetchone()
    ...
    def record_use(self, credential_id: str, sign_count: int) -> None:
        ...
        with self._connect() as connection:
            connection.execute(
                ...
                (max(0, int(sign_count)), _stamp(), credential_id),
            )
    ...
    def verified_recently(self, device_id: str) -> bool:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT second_factor_at FROM devices WHERE device_id = ?", (device_id,)
            ).fetchone()
    ...
    def clear_verification(self, device_id: str) -> None:
        with self._connect() as connection:
            connection.execute(
                "UPDATE devices SET second_factor_at = NULL WHERE device_id = ?",
                (device_id,),
            )
