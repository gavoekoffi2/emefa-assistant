...
def _scoped_factors(request: Request, principal: Account | Device):
    return request.app.state.second_factor.for_scope(
...
def require_recent_step_up(request: Request, device: Device) -> None:
    ...
    factors = _scoped_factors(request, device)
    if (
        device.account_id
...
def status(
    ...
    factors = _scoped_factors(request, account)
